...
void MobilityBase::handlePositionUpdate(Coord newPosition)
{
    lastPosition = newPosition;
    checkPosition();
    emitMobilityStateChangedSignal();
...
void MobilityBase::readPositionFromFile(const char *filename, int key)
{
    std::ifstream file(filename);
    if (!file.is_open()) {
        throw cRuntimeError("Unable to open file: %s", filename);
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (lineStream >> id && lineStream.get() == ':' && lineStream >> x >> y >> z) {
            ...
            std::cerr << "wrong fromat: " << line << std::endl;
        }
    }
